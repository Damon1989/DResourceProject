...
namespace DResourceProject.Tests
{
    public abstract class DResourceProjectTestBase : AbpIntegratedTestBase<DResourceProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected DResourceProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<DResourceProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DResourceProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DResourceProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DResourceProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DResourceProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DResourceProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DResourceProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DResourceProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DResourceProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DResourceProjectDbContext, Task<T>> func)
        {
            T result;
